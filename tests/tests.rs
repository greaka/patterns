...
fn basic() {
    for i in 0..63 {
        with_misaligned(&[0x42], i, |data| {
            assert_eq!(simple("42", &data[0..1]), &[0]);
            assert_eq!(simple("24", &data[0..1]), &[]);
            assert_eq!(simple("42", data), &[0, 1]);
        })
    }
}
...
fn leading_wildcard() {
    for i in 0..63 {
        with_misaligned(&[0x42], i, |data| {
            assert_eq!(simple("? 42", data), &[]);
        });
    }
    for i in 0..63 {
        with_misaligned(&[0x22, 0x42], i, |data| {
            assert_eq!(simple("? 42", data), &[0]);
        });
    }
}
...
fn trailing_zero() {
    for i in 0..63 {
        with_misaligned(&[0x42], i, |data| {
            assert_eq!(simple("00", data), &[]);
            assert_eq!(simple("42 00", data), &[]);
        });
        with_misaligned(&[0x00], i, |data| {
            assert_eq!(simple("00", data), &[0]);
        });
        with_misaligned(&[0x42, 0x00], i, |data| {
            assert_eq!(simple("42 00", data), &[0]);
        });
    }
}
...
fn overlap() {
    for i in 0..63 {
        with_misaligned(&[0xab, 0xcd, 0xab, 0xcd, 0xab, 0xcd], i, |data| {
            assert_eq!(simple("ab ?? ?? cd", data), &[0, 2]);
            assert_eq!(simple("ab ?? ??", data), &[0, 2]);
            assert_eq!(simple("?? ?? cd", data), &[1, 3]);
        });
    }
}
