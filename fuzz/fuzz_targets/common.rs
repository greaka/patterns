...
#[derive(Debug)]
pub struct FuzzData(String, Vec<u8>, u8);
...
pub fn with_misaligned<F: FnOnce(&[u8]) -> T, T>(data: &[u8], offset: usize, f: F) -> T {
...
pub fn hex_list<T: std::fmt::LowerHex>(list: &[T]) -> String {
    format!(
        "[{}]",
        list.iter()
...
#[rustfmt::skip]
pub fn repro(input: &FuzzData, alignment: u8, bytes: u8, reference: &[usize]) -> String {
